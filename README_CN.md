...
Flutter 桥接 `0.7.1` 固定要求 Flutter `3.22.3`，iOS 制品仅通过 CocoaPods 交付，
...
`jolibox_ads_flutter (0.7.1)` 与 `Google-Mobile-Ads-SDK (12.1.0)`。不要仅为改变版本而
...
默认模式下，Banner 在原生 View 已加载、挂载并具有非零布局尺寸前不占用页面高度；
加载失败也不会留下空 Banner 位。加载成功后默认立即恢复完整高度；只有宿主显式指定时
才执行展开动画。这是新宿主推荐使用的模式。
...
`revealDuration` 是可选参数，默认值为 `Duration.zero`。它仅对
`collapseUntilLoaded` 生效：传入零时立即恢复 Banner 高度；传入任意正数 Dart
`Duration` 时按指定时长展开。例如：
...
`onLoaded` 在原生 Banner View 已挂载且具有非零布局尺寸后调用，并早于可选展开动画
完成；它不保证原生广告首个像素已经绘制。展开动画不会延迟 `onLoaded`。
